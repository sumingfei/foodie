	...
	*/
	function ProfileRequestScriptSessionKey($auth, $expires, $consumeWithin, $permittedReferrerRegex, $cookie, &$sessionKey){
		...
		if(!empty($auth['user_id'])){
			...
			$this->ProfileGetAuth($auth['user_id'], $token2, $secret2);
				$auth['token'] = $token2;
				$auth['secret'] = $secret2;
		}
	...
	function getFood($foodID){
		$url = FatSecretAPI::$base . 'method=food.get&format=json&food_id=' . $foodID;
	...
	private function GetQueryResponse($requestUrl, $postString) {
		$ch = curl_init();
/*		print '<div>: requestURL</div>';
		echo $requestUrl;
		print '<div>: postString</div>';
		echo $postString;
		print '<div>: end</div>';*/
...
//		print '<div>timestamp: ' . $timeStamp . '</div>';
//		print '<div>nonce: ' . $nonce . '</div>';
